...
const BiasConfigPopup = ({ 
  ...
  const sharingURL = `${baseURL}/pages/biasMapping?sessionID=${sessionID}&biasNumber=${biasNumber}&prePopulate=${prePopulate}`;
  ...
  const handleStart = () => {
    const url = `${baseURL}/pages/biasMapping?sessionID=${sessionID}&biasNumber=${biasNumber}` + 
      (prePopulate ? '&prePopulate=true' : '');
    router.push(url);
  ...
  const handleModeSelect = (selectedMode) => {
    if (selectedMode === 'individual') {
      ...
      router.push(`${baseURL}/pages/biasMapping?sessionID=group1&biasNumber=5&individualMode=true`);
      onClose?.();
    } else {
