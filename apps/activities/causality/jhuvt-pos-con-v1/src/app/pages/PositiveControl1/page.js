...
function PositiveControlContent() {
  ...
  const defaultFlowId = '6842fc7d81fa9b6e13bda8ab';
  ...
  const saveCurrentSessionFlow = useCallback(async () => {
    if (!flowIdFromUrl || !selectedFlowInfo) return;
    ...
    try {
      console.log(`[SessionFlow] Saving session ${sessionId} for flow ${flowIdFromUrl}`);
      ...
      if (!result.success) {
        ...
        console.log(`[SessionFlow] Successfully saved session ${sessionId}`);
      }
    } catch (error) {
  ...
  }, [setEdges, nodes, sessionId, flowIdFromUrl, selectedFlowInfo, validations]);
  ...
  const onDrop = useCallback(
    ...
    [reactFlowInstance, setNodes, edges, sessionId, flowIdFromUrl, selectedFlowInfo, validations]
  );
  ...
  const handleSubmitPositiveControlWork = async () => {
    ...
    console.log(`[PositiveControl1] Submitting data with sessionId: ${sessionId}`);
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/pages/PositiveControl2?flowId=${flowIdFromUrl}&sessionID=${sessionId}`);
      } else {
    ...
    return (
      <div className="positive-control-page">
        <div className="loading-indicator">
          ...
          <p>Loading Positive Control 1...</p>
        </div>
      </div>
    );
  }
