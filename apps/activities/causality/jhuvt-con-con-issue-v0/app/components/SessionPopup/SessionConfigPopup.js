...
const SessionConfigPopup = ({ 
  ...
  const handleModeSelect = (selectedMode) => {
    if (selectedMode === 'individual') {
      // Route to individual mode with hardcoded sessionID
      router.push(`/review-controls?sessionID=individual1`);
      handleClose();
    } else {
      // Generate new sessionID for group mode
      ...
      const sharingURL = `${window.location.origin}/review-controls?sessionID=${newSessionID}`;
  ...
  const handleStart = () => {
    router.push(`/review-controls?sessionID=${sessionID}`);
    handleClose();
  };

  const handleCopyLink = async () => {
    try {
      const sharingURL = `${window.location.origin}/review-controls?sessionID=${sessionID}`;
      await navigator.clipboard.writeText(sharingURL);
    } catch (err) {
      logger.app.error('Failed to copy:', err);
    }
            ...
            <Box sx={{ 
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Typography variant="body1" sx={{ 
                  ...
                  {`${window.location.origin}/review-controls?sessionID=${sessionID}`}
                </Typography>
                <Button 
                  variant="contained"
                  onClick={handleCopyLink}
