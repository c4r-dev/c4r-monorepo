...
function ActivityContent() {
  ...
  useEffect(() => {
    if (!sessionIdFromUrl) {
      // No sessionID in URL, generate a new one and refresh
      const newSessionId = generateSessionId();
      const currentUrl = new URL(window.location);
      currentUrl.searchParams.set('sessionID', newSessionId);
      
      // Preserve existing flowId if present
      if (flowIdFromUrl) {
        currentUrl.searchParams.set('flowId', flowIdFromUrl);
      }
      
      // Replace the current URL with the new sessionID
      router.replace(currentUrl.pathname + currentUrl.search);
      return;
    } else {
      // SessionID exists, store it in state
      setSessionId(sessionIdFromUrl);
    }
  }, [sessionIdFromUrl, flowIdFromUrl, router]);
