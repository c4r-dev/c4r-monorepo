...
const SimpleCustomNode = ({ id, data, selected, isConnectable }) => {
  ...
  const fontSize = data.fontSize || 14; // Default 14px
  ...
  const isRotated = data.isRotated || false;
  const inputPosition = isRotated ? Position.Top : Position.Left;
  const outputPosition = isRotated ? Position.Bottom : Position.Right;
  ...
  // Trigger edge update when text changes (debounced for performance)
  useEffect(() => {
    if (isEditing && data.updateLabel) {
      const timeoutId = setTimeout(() => {
        // Only update if we're still editing and value has changed
        if (editValue !== (data.label || 'Node')) {
          data.updateLabel(id, editValue);
        }
      }, 300); // 300ms debounce
    ...
    <div className={nodeClasses} style={nodeStyle}>
      <NodeToolbar isVisible={selected || isEditing} position={Position.Top} aria-label="Node toolbar">
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button
            ...
            onClick={() => {
              const newSize = Math.max(8, (data.fontSize || 14) - 2);
              data.updateStyle && data.updateStyle(id, { fontSize: newSize });
            }}
          ...
          <span style={{ fontSize: '12px', color: '#666', minWidth: '24px', textAlign: 'center' }}>
            {fontSize}px
          </span>
          <button
            className="btn btn--secondary"
            onClick={() => {
              const newSize = Math.min(32, (data.fontSize || 14) + 2);
              data.updateStyle && data.updateStyle(id, { fontSize: newSize });
            }}
          ...
          <button
            className="btn btn--secondary"
            onClick={() => data.rotateNode && data.rotateNode(id)}
            aria-label="Rotate node handles"
            type="button"
            title={`Switch to ${isRotated ? 'left/right' : 'top/bottom'} handles`}
          >
            ↻
          </button>
          <button
      ...
      {isEditing ? (
        <textarea
          ...
          style={{ height: 'auto', fontSize: `${fontSize}px` }}
        />
      ) : (
        <div 
...
function DeletableSmoothEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, style, markerEnd, selected, data }) {
  ...
  // Use different marker for selected vs unselected edges
  const effectiveMarkerEnd = selected ? 'url(#custom-arrow-selected)' : 'url(#custom-arrow)';
  
  // Enhanced style for better visibility
...
function DesignerActivityPage({ sessionIdFromUrl }) {
  ...
  // Delete an edge by id (used by custom edge delete button and elsewhere)
  const deleteEdgeById = useCallback((edgeId) => {
    setEdges((existingEdges) => existingEdges.filter((edge) => edge.id !== edgeId));
    setLiveMessage('Connection deleted');
  }, [setEdges, setLiveMessage]);

  // Force re-render of edges connected to a specific node
  const forceEdgeUpdate = useCallback((nodeId) => {
    setEdges((existingEdges) => 
      existingEdges.map((edge) => {
        if (edge.source === nodeId || edge.target === nodeId) {
          // Force re-render by updating a timestamp property
          return { 
            ...edge, 
            data: { 
              ...edge.data, 
  ...
  // Custom nodes change handler that also triggers edge updates when needed
  const handleNodesChange = useCallback((changes) => {
    onNodesChange(changes);
    
    // Check if any changes involve dimension/position updates that might affect edge positioning
    changes.forEach((change) => {
      if (change.type === 'dimensions' || change.type === 'position') {
        // Small delay to ensure node updates are processed first
        setTimeout(() => {
          forceEdgeUpdate(change.id);
        }, 10);
      }
    });
  }, [onNodesChange, forceEdgeUpdate]);
  ...
  const updateNodeLabel = useCallback((nodeId, newLabel) => {
    ...
    // Force edge re-render since text changes might affect node size/handle positions
    forceEdgeUpdate(nodeId);
  }, [setNodes, forceEdgeUpdate]);
  ...
  const rotateNodeById = useCallback((nodeId) => {
    setNodes((nds) =>
      nds.map((node) => {
        if (node.id === nodeId) {
          const wasRotated = node.data.isRotated || false;
          return {
            ...node,
            // Use a unique key to force complete re-render
            key: `${nodeId}-${Date.now()}`,
            data: {
              ...node.data,
              isRotated: !wasRotated,
              // Force handle position recalculation
    ...
    // Multiple approaches to force ReactFlow to update handle positions
    setTimeout(() => {
      // Force edge re-render
      forceEdgeUpdate(nodeId);
      
      // Force ReactFlow instance to update
      if (rfInstanceRef.current) {
        // Get current viewport and apply a tiny change to force refresh
        const viewport = rfInstanceRef.current.getViewport();
        rfInstanceRef.current.setViewport({
          ...viewport,
          zoom: viewport.zoom + 0.000001 // Tiny change to force update
        });
        setTimeout(() => {
          rfInstanceRef.current.setViewport(viewport); // Restore original
        }, 10);
      }
    }, 10);

    setLiveMessage('Node rotated');
  }, [setNodes, forceEdgeUpdate]);
  ...
  const injectHandlers = useCallback((node) => ({
    ...
    data: {
      ...
      rotateNode: rotateNodeById,
    },
  }), [updateNodeLabel, updateNodeStyle, deleteNodeById, rotateNodeById]);
      ...
      if (Array.isArray(savedNodes) && savedNodes.length) {
        ...
        const normalized = savedNodes.map((n) => ({
          ...
          data: {
            ...
            isRotated: n?.data?.hasOwnProperty('isRotated') ? n.data.isRotated : true, // Default to top/bottom orientation
          },
        }));
        ...
        const normalizedEdges = (Array.isArray(savedEdges) ? savedEdges : []).map((e) => ({
          ...e,
          type: 'deletableEdge',
          markerEnd: e.markerEnd || 'url(#custom-arrow)',
          style: e.style || { 
            strokeWidth: 3,
            stroke: '#666'
          },
  ...
  useEffect(() => {
    if (phase === 2) {
      const serializableNodes = nodes.map((n) => ({ 
        ...n, 
        key: undefined, // Remove React key
        data: { 
          ...n.data, 
          updateLabel: undefined, 
          updateStyle: undefined, 
          deleteNode: undefined, 
          rotateNode: undefined, 
          handlePositionsUpdated: undefined 
        } 
      }));
  ...
  // Ensure arrow markers are visible and properly sized
  useEffect(() => {
    const addCustomMarkers = () => {
      ...
      const existingMarkers = defs.querySelectorAll('marker[id*="custom-arrow"]');
      existingMarkers.forEach(marker => marker.remove());

      // Create large, visible arrow marker
      ...
      path.setAttribute('d', 'M2,2 L2,18 L18,10 z');
      ...
      // Create selected state marker (larger and blue)
      const selectedMarker = document.createElementNS('http://www.w3.org/2000/svg', 'marker');
      selectedMarker.setAttribute('id', 'custom-arrow-selected');
      selectedMarker.setAttribute('markerWidth', '24');
      selectedMarker.setAttribute('markerHeight', '12');
      selectedMarker.setAttribute('refX', '22');
      selectedMarker.setAttribute('refY', '12');
      selectedMarker.setAttribute('orient', 'auto');
      selectedMarker.setAttribute('markerUnits', 'strokeWidth');
      selectedMarker.setAttribute('viewBox', '0 0 24 24');

      const selectedPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      selectedPath.setAttribute('d', 'M2,2 L2,22 L22,12 z');
      selectedPath.setAttribute('fill', '#0078d4');
      selectedPath.setAttribute('stroke', '#0078d4');
      selectedPath.setAttribute('stroke-width', '1');
      
      selectedMarker.appendChild(selectedPath);
      defs.appendChild(selectedMarker);
    };

    if (phase === 2) {
      // Add markers after a short delay to ensure ReactFlow is fully rendered
      setTimeout(addCustomMarkers, 100);
      // Also add them when the component updates
      const interval = setInterval(addCustomMarkers, 500);
  ...
  }, [setEdges, deleteEdgeById, setLiveMessage]);
  ...
  const addNewNode = useCallback(() => {
    const newNode = {
      ...
      data: {
        ...
        textColor: '#212529',
        fontSize: 14,
        isRotated: true, // Default to top/bottom orientation
        updateLabel: updateNodeLabel,
        updateStyle: updateNodeStyle,
        deleteNode: deleteNodeById,
        rotateNode: rotateNodeById,
  ...
  }, [setNodes, nodes.length, updateNodeLabel, updateNodeStyle, deleteNodeById, rotateNodeById]);
  ...
  const handleSubmitDesign = useCallback(async () => {
    ...
    const sanitizedNodes = nodes.map((n) => ({
      ...
      dragging: false,
      key: undefined, // Remove React key
      data: { ...n.data, updateLabel: undefined, updateStyle: undefined, deleteNode: undefined, rotateNode: undefined, handlePositionsUpdated: undefined },
    }));
  ...
  return (
    <div className="designer-activity-container">
      <header className="submitted-designs-header">
        ...
        <div className="submitted-designs-actions">
          <button
