// Below are some functions that are used in the PositiveControl1 page.
// They maye be useful for implementing the PositiveControl2 page with some modifications.
...
// PositiveControl2 validation utilities
...
// Load submissions for a specific session
export const loadSubmissionsForSession = async (sessionId) => {
  try {
    const response = await fetch(`/api/positive-control-submissions/session/${sessionId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
