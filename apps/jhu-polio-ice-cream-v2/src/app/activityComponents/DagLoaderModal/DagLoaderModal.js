...
const SaveRestore = (flowObjectNested) => {

    logger.app.info("flowObjectNested in DagLoaderModal", flowObjectNested);

    const flowObject = flowObjectNested.flowObject.flowObject;
    const flowDescription = flowObject.description;
    const closeDagModal = flowObject.closeDagModal;
    logger.app.info("flowObject in DagLoaderModal", flowObject);
    logger.app.info("flowDescription in DagLoaderModal", flowDescription);
    ...
    const addNodeToCanvas = useCallback((variableName) => {
        const newNode = {
            ...
            data: { label: variableName,
                // Set edge marker to arrow closed
                markerEnd: {
            ...
            >
                ...
                >
                    <Panel position="top-right">
                        {/* <div className="expand-dag-button">
                            <button onClick={closeDagModal}>Collapse</button>
                        </div> */}
                    </Panel>
...
export default function DagLoaderModal(flowObject, flowDescription, closeDagModal ) {
    return (
        <ReactFlowProvider style={{width: "100%", height: "100%"}}>
            <SaveRestore flowObject={flowObject} flowDescription={flowDescription} closeDagModal={closeDagModal} />
        </ReactFlowProvider>
    );
}
