...
export default function LoadDagGroup() {
    ...
    useEffect(() => {
        ...
        if (dagResults.length > 4) {
            ...
            const dagResultsForLabGroup = dagResults.filter(dag => dag.groupId === labGroupId);
            ...
            // Asign the 4 most recent dagResults to flowObject, flowObject2, flowObject3, and flowObject4
            setFlowObject(JSON.parse(sortedDagResults[0].flow));
            setFlowObject2(JSON.parse(sortedDagResults[1].flow));
            setFlowObject3(JSON.parse(sortedDagResults[2].flow));
            setFlowObject4(JSON.parse(sortedDagResults[3].flow));
            setFlowDescription1(sortedDagResults[0].description);
            setFlowDescription2(sortedDagResults[1].description);
            setFlowDescription3(sortedDagResults[2].description);
            setFlowDescription4(sortedDagResults[3].description);
            // logger.app.info("flowDescription1:", flowDescription1);
            // logger.app.info("flowDescription2:", flowDescription2);
            // logger.app.info("flowDescription3:", flowDescription3);
            // logger.app.info("flowDescription4:", flowDescription4);
    ...
    return (
        <div className="dag-results-page">
            ...
            <div className="dag-results-page-container">
                ...
                {showMore && (
                    <div className="dag-loader-container">
                        {labGroupFlowObjects.map((flowObject, index) => (
                            <DagLoader
                                key={index}
                                className="dag-loader"
                                flowObject={flowObject}
                                flowDescription={labGroupFlowDescriptions[index]}
                                openDagModal={openDagModal}
                            />
                        ))}
                    </div>
