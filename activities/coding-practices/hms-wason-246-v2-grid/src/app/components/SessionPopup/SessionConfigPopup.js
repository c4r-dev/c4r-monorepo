...
const SessionConfigPopup = ({ 
  ...
  // Check for sessionID in URL on mount - but don't auto-close if popup should be open
  useEffect(() => {
    const urlSessionID = searchParams.get('sessionID');
    if (!urlSessionID && !open) {
      onClose?.();
    }
  }, [searchParams, open]);
  ...
  const handleClose = (event, reason) => {
    ...
    setMode(null);
    setQrCodeUrl('');
    onClose?.(); // No mode parameter for regular close
  };
  ...
  const handleStart = () => {
    ...
    onClose?.(); // No mode parameter for group start
  };
