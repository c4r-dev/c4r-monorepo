...
function RandomizeActivityVariablesContent(props) {
 ...
 const searchParams = useSearchParams()
  ...
  const handleSpin = async (grp_id) => {
    const response = await fetch('https://smi-ran-why-ran-v4.vercel.app/api/firstUsers', {
      method: 'POST',
      headers: {
...
function RandomizeActivityVariables() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <RandomizeActivityVariablesContent />
    </Suspense>
  )
}
